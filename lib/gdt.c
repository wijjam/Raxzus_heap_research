...
// Static kernel stack for the TSS.  Must NOT use kmalloc — the heap does
// not exist yet when init_gdt() runs.
...
void init_gdt(void)
{
    ...
    tss_set_entry(5, (uint32_t)tss_kernel_stack + sizeof(tss_kernel_stack));
